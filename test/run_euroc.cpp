...
string	sData_path = "/media/hy/WD/Data/slam/EuRoc/mv0/";
...
void PubImuData()
{
	...
	while (std::getline(fsImu, sImu_line) && !sImu_line.empty()) // read imu data
	{
		...
		usleep(5000*nDelayTimes);  // 把调用该函数的线程挂起一段时间，单位是微秒（百万分之一秒）
	}
	fsImu.close();
}
...
// 如果程序中定义了 __APPLE__ 这个符号，则编译下列语句（#endif 或者#else之前的) 
#ifdef __APPLE__
// support for MacOS
...
int main(int argc, char **argv)
{
	...
	std::thread thd_BackEnd(&System::ProcessBackEnd, pSystem);  // 处理　主程序　， imu, img
