...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)
    {
        ...
        int j = frame_count;
        Vector3d un_acc_0 = Rs[j] * (acc_0 - Bas[j]) - g;       // 加速度 k 测量值
        Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - Bgs[j]; // 中值法 离散积分： 角速度 w

        Rs[j] *= Utility::deltaQ(un_gyr * dt).toRotationMatrix();   //q： 旋转矩阵，  增量采用 四元数描述,虚部： 0.5*w*delt_t 
        Vector3d un_acc_1 = Rs[j] * (linear_acceleration - Bas[j]) - g;  // 加速度 k+1 测量值 // k+1 时刻涉及 q_k+1 时刻更新
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);            // 中值积分 离散积分： 加速度 a
        
        Ps[j] += dt * Vs[j] + 0.5 * dt * dt * un_acc;               // p:
        Vs[j] += dt * un_acc;                                       // v:
    }
    acc_0 = linear_acceleration;     // k 加速度 
    gyr_0 = angular_velocity;        // k 角速度 测量值
}
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    // ×××××××两种　marg 的处理，　对关键帧的判断
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))//关键帧　判断函数
    ...
    if (ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            vector<pair<Vector3d, Vector3d>> corres = f_manager.getCorresponding(frame_count - 1, frame_count); // 把特征点配对
            Matrix3d calib_ric;    
            // ×××× 1） ： 估计旋转外参 q_bc
    ...
    if (solver_flag == INITIAL)// 初始化
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if (ESTIMATE_EXTRINSIC != 2 && (header - initial_timestamp) > 0.1)
            {
                ...
                result = initialStructure();///   初始化　５　步
                initial_timestamp = header;
            }
            if (result)
            {
                solver_flag = NON_LINEAR;
                solveOdometry();// ×××××××××××　里程计　　　后端优化　backend   marg: old, new
                slideWindow();// 滑窗　
...
bool Estimator::initialStructure()
{
    ...
    for (int i = 0; frame_it != all_image_frame.end(); frame_it++)
    {
        ...
        cv::Mat K = (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, 1, 0, 0, 0, 1);  // 内参K
...
bool Estimator::visualInitialAlign()
{
    ...
    ric[0] = RIC[0];
    f_manager.setRic(ric);
    f_manager.triangulate(Ps, &(TIC_TMP[0]), &(RIC[0]));// 三角化　求解　所有相机　特征点深度

    double s = (x.tail<1>())(0);
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        pre_integrations[i]->repropagate(Vector3d::Zero(), Bgs[i]);// 构造　状态转移方程（协方差矩阵)
    }
    ...
    R0 = Utility::ypr2R(Eigen::Vector3d{-yaw, 0, 0}) * R0;  // 三轴旋转矩阵　Rz * Ry * Rx;
    g = R0 * g;
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)
        {
            ...
            for (int j = 0; j < int(corres.size()); j++)
            {
                ...
                double parallax = (pts_0 - pts_1).norm();  // F范数
...
void Estimator::vector2double()
{
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        ...
        para_Pose[i][4] = q.y();
        para_Pose[i][5] = q.z();
        para_Pose[i][6] = q.w();  //double 姿态　数组　11*7 (滑窗数量size * [x,y,z, x,y,z,w]
        ...
        para_SpeedBias[i][8] = Bgs[i].z(); //　double 滑窗内　速度　噪声　　11*9
    }
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        para_Ex_Pose[i][6] = q.w();     // 每个相机的外参
    }

    VectorXd dep = f_manager.getDepthVector();      // 逆深度向量
...
bool Estimator::failureDetection()
{
    ...
    */
    ...
    double delta_angle;
    delta_angle = acos(delta_Q.w()) * 2.0 / 3.14 * 180.0;   //四元数虚部作为旋转角度评价
    if (delta_angle > 50)
    {
...
void Estimator::MargNewFrame()  //直接传到下一帧，构建Hessin没有视觉约束
{
    ...
    vector<shared_ptr<backend::VertexInverseDepth>> vertexPt_vec;
    {
        ...
        for (auto &it_per_id : f_manager.feature)
        {
            ...
            for (auto &it_per_frame : it_per_id.feature_per_frame)
            {
                ...
                Vector3d pts_j = it_per_frame.point;
                                                            // 对每一个点　计算视觉重投影误差，　及误差对状态的　Jacobian
                std::shared_ptr<backend::EdgeReprojection> edge(new backend::EdgeReprojection(pts_i, pts_j));
                std::vector<std::shared_ptr<backend::Vertex>> edge_vertex;
                edge_vertex.push_back(verterxPoint);
                edge_vertex.push_back(vertexCams_vec[imu_i]);
