...
void System::PubImageData(double dStampSec, Mat &img)
{
    ...
    // ********** 主要函数***********//
    trackerData[0].readImage(img, dStampSec);// 读取图片并调整　　直方图均衡，调整，
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    double x = un_pts[j].x;
                    double y = un_pts[j].y;
                    double z = 1;           // 归一化无畸变图像坐标
                    feature_points->points.push_back(Vector3d(x, y, z));
                    feature_points->id_of_point.push_back(p_id * NUM_OF_CAM + i);
                    feature_points->u_of_point.push_back(cur_pts[j].x);
                    feature_points->v_of_point.push_back(cur_pts[j].y);
                    feature_points->velocity_x_of_point.push_back(pts_velocity[j].x);// 这里不考虑　像素速度（秦通）
            ...
            {
                m_buf.lock();
                feature_buf.push(feature_points);// 特征点　队列buf
...
{
    shared_ptr<IMU_MSG> imu_msg(new IMU_MSG());//自定义　类　　IMU_MSG
    ...
    m_buf.lock();
    imu_buf.push(imu_msg);      // 把高频imu塞进队列，根据时间戳　查到　与图像挂钩的　imu数据
    // cout << "1 PubImuData t: " << fixed << imu_msg->header 
...
void System::ProcessBackEnd()
{
    ...
    while (bStart_backend)
    {
        ...
        con.wait(lk, [&] {                              // 等待　数据进行“时间上的对齐
            return (measurements = getMeasurements()).size() != 0;  // 读取　　img_buf, imu_buf  序列数据：imu组－img
        });
        ...
        lk.unlock();
        m_estimator.lock();
        for (auto &measurement : measurements)  // 　处理　imu    img
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                else  // 根据时间　插值　，再　预积分
                {
                    ...
                    double dt_2 = t - img_t;
                    current_time = img_t;
                    assert(dt_1 >= 0);　// 如果它的条件返回错误，则终止程序执行。
                    assert(dt_2 >= 0);
                    assert(dt_1 + dt_2 > 0);
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++) 
            {
                ...
                image[feature_id].emplace_back(camera_id, xyz_uv_velocity);   // 只调用转移构造函数，在插入的时候直接构造，效率更高，减少额外空间的开辟
            }
            TicToc t_processImage;
            estimator.processImage(image, img_msg->header);// 处理　主要　步骤
