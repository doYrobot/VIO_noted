...
//    std::cout << pts_i_.transpose() <<" "<<pts_j_.transpose()  <<std::endl;
    ...
    Qd Qi(param_i[6], param_i[3], param_i[4], param_i[5]);  // i时刻　imu　姿态Ｒ_wbi
    Vec3 Pi = param_i.head<3>();        // i 位置　ｔ

    VecX param_j = verticies_[2]->Parameters();     //      j 时刻　　姿态R_wbj
    Qd Qj(param_j[6], param_j[3], param_j[4], param_j[5]);
    Vec3 Pj = param_j.head<3>();

    VecX param_ext = verticies_[3]->Parameters();       // 相机　到　IMU 的外参　　
    Qd qic(param_ext[6], param_ext[3], param_ext[4], param_ext[5]);// R_bc
    Vec3 tic = param_ext.head<3>();                                 // t

    Vec3 pts_camera_i = pts_i_ / inv_dep_i;             //  归一化相机系下　坐标
    Vec3 pts_imu_i = qic * pts_camera_i + tic;          //  imu body系下　坐标
    Vec3 pts_w = Qi * pts_imu_i + Pi;                   //　　在　ｋ时刻的世界坐标
    Vec3 pts_imu_j = Qj.inverse() * (pts_w - Pj);       //　　重新投影到　j　时刻　imu  body系
    Vec3 pts_camera_j = qic.inverse() * (pts_imu_j - tic); //　投影到　　j        cam 系

    double dep_j = pts_camera_j.z();
    residual_ = (pts_camera_j / dep_j).head<2>() - pts_j_.head<2>();        // 残差ｒ　像素误差
      /// J^t * J * delta_x = - J^t * r
...
void EdgeReprojection::ComputeJacobians() {
    ...
    reduce << 1. / dep_j, 0, -pts_camera_j(0) / (dep_j * dep_j),  
        0, 1. / dep_j, -pts_camera_j(1) / (dep_j * dep_j);      // 残差　对　f_cj　，（链式求导），要乘以后面每一项
//    reduce = information_ * reduce;
    ...
    Eigen::Matrix<double, 2, 6> jacobian_ex_pose;       //  f_cj  对　imu和cam间的外参求导
    Eigen::Matrix<double, 3, 6> jaco_ex;
    jaco_ex.leftCols<3>() = ric.transpose() * (Rj.transpose() * Ri - Eigen::Matrix3d::Identity());

    Eigen::Matrix3d tmp_r = ric.transpose() * Rj.transpose() * Ri * ric;    // 中间变量
    jaco_ex.rightCols<3>() = -tmp_r * Utility::skewSymmetric(pts_camera_i) + Utility::skewSymmetric(tmp_r * pts_camera_i) +     // part 1
                             Utility::skewSymmetric(ric.transpose() * (Rj.transpose() * (Ri * tic + Pi - Pj) - tic));           // part 2
