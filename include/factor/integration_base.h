...
class IntegrationBase
{
  public:
    ...
    // 系统　误差传递的　状态转移方程　 sigama_k = F *sigama_k-1 * F^T + G * sigama_n * G^T
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
            // q 预积分
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);  // q_wbk+1

        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);  //　中值　加速度　ａ
            // p 预积分
        ...
        if(update_jacobian)     //  协方差传递矩阵
        {
            ...
            R_w_x<<0, -w_x(2), w_x(1),      // 角速度的　外积
                w_x(2), 0, -w_x(0),
                -w_x(1), w_x(0), 0;
            R_a_0_x<<0, -a_0_x(2), a_0_x(1),    // 两个时刻的　加速度　外积
                a_0_x(2), 0, -a_0_x(0),
                -a_0_x(1), a_0_x(0), 0;
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        //                    linearized_ba, linearized_bg);
        delta_p = result_delta_p;  // 更新　状态量　ｐｑｖ　    ba bg  更新了么？
